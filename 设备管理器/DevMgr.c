...
//
INT_PTR CALLBACK MainDialog(HWND hDlg, UINT uMsg,WPARAM wParam, LPARAM lParam)
{
        ...
        case WM_COMMAND:
        {
            switch(LOWORD(wParam))
            {
                case IDC_ADDBTN:
                {
                    short wLegacy = 0;
//
                    if (SendMessage(GetDlgItem(hDlg, IDC_RADIO2),BM_GETCHECK, 0, 0) == BST_CHECKED)
                        wLegacy = 1;
                    if (DialogBoxParam(_hInst, MAKEINTRESOURCE(IDD_DIALOG2),hDlg, InstallDialog, wLegacy) == -1)
...
//
                        if (ListViewGetItemSelect(hList) == 10 ||ListViewGetItemSelect(hList) == 11)
                        {
...
int APIENTRY WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmdLine, int nShowCmd)
{
    InitCommonControls();
    _hInst = hInst;
    if (DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_DIALOG1),0L, MainDialog, 0L) == -1)
