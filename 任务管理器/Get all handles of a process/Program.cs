...
namespace Get_all_handles_of_a_process
{
    class Program
    {
        [DllImport("ntdll.dll")]
        ...
        static void Main(string[] args)
        {
            ...
            var tmp = Win32Processes.GetProcess();
            foreach (var VARIABLE in tmp)
            {
                int nLength = VARIABLE.ImageName.Length;
                byte[] baTemp = new byte[nLength];
                IntPtr ptr = (IntPtr) VARIABLE.ImageName.Buffer;
                if (ptr!=IntPtr.Zero)
                {
                    Marshal.Copy(ptr, baTemp, 0, nLength);
                    string str = Encoding.Unicode.GetString(baTemp);
                    Console.WriteLine(VARIABLE.UniqueProcessId + "\t\t" + str);
                }
                else
                {
                    Console.WriteLine(VARIABLE.UniqueProcessId + "\t\t" + "" );
                }
            }
    ...
    public class Converter
    {
        ...
        public static Object BytesToStruct(Byte[] bytes, Type strcutType)
        {
            unsafe
            {
                Int32 size = sizeof(Win32Processes.SYSTEM_PROCESS_INFORMATION);
                IntPtr buffer = Marshal.AllocHGlobal(size);
                try
                {
